...
def force_counterclockwise(points):
    """
...
def adjust_points_gap(shp, gap, image_top=0):
    """
    ...
    pts = shp["points"]
    if len(pts) < 2:
        assert False, "len(shp['points']) < 2"

    # Determine if this shp is a closed polygon (3+ points).
    is_closed = False
    if len(pts) >= 3:
        is_closed = True
    ...
    if is_closed and len(new_points) >= 3:
        new_points = force_counterclockwise(new_points)
...
if __name__ == "__main__":
    ...
    for datafile in datafiles:
        ...
        task4_samedist_gap_dir = datafile['task4_1_samedist_gap_dir']
        Path(task4_samedist_gap_dir).mkdir(exist_ok=True, parents=True)

        img_fps = sorted(glob.glob(os.path.join(imgmask_filtered_dir, "*.jpg")))[:10]
        get_mask_fp = lambda img_fp: img_fp.replace(".jpg", ".json")
        img_mask_fps = [(img_fp, get_mask_fp(img_fp)) for img_fp in img_fps]

        for idx, (img_fp, mask_fp) in enumerate(img_mask_fps):
            if idx % 100 == 0:
                print(idx, "/", len(img_mask_fps))  

            h, w = get_image_size(img_fp)
            gap = datafile['gap']/336*np.sqrt(h * w)
            mask_fp_dst = os.path.join(task4_samedist_gap_dir, os.path.basename(mask_fp))
            ...
            for shp in mask_data["shapes"]:
                shp["points"] = adjust_points_gap(shp, gap=gap)

            # Write the updated annotation to the output file.
            with open(mask_fp_dst, 'w') as f:
