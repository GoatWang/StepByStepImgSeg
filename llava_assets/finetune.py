...
def train(attn_implementation=None):
    ...
    if training_args.bits in [4, 8]:  # (bits = 16) not go this branch
    ...
    if model_args.vision_tower is not None:
        if "mpt" in model_args.model_name_or_path:
            ...
            model = LlavaLlamaForCausalLM.from_pretrained(  # this branch
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
    ...
    if training_args.lora_enable:  # Jeremy: inside
        ...
        lora_config = LoraConfig(
            ...
            target_modules=find_all_linear_names(
                model
            ),  # ['k_proj', 'q_proj', 'o_proj', 'down_proj', 'gate_proj', 'up_proj', 'v_proj']
    ...
    # clip_vision_tower = CLIPVisionTower("openai/clip-vit-large-patch14-336")
    # clip_vision_tower.to(
    #     dtype=torch.bfloat16 if training_args.bf16 else torch.float16,
    #     device=training_args.device,
    # )

    # os.environ["CUDA_LAUNCH_BLOCKING"] = "1"
    # os.environ["TORCH_USE_CUDA_ASSERTIONS"] = "1"
    # clip_vision_tower = model.get_vision_tower()
    # clip_vision_tower.to(
    #     dtype=torch.bfloat16 if training_args.bf16 else torch.float16,
    #     device=training_args.device,
    # )

    # images = torch.zeros((4, 3, 336, 336))
    # clip_vision_tower.vision_tower(
    #     images.to(device=clip_vision_tower.device, dtype=clip_vision_tower.dtype),
...
def main():
    ...
    print("torch.version.cuda", torch.version.cuda)  # PyTorch's CUDA version
    print("torch.cuda.is_available", torch.cuda.is_available())  # Should be True
    print("torch.cuda.device_count", torch.cuda.device_count())  # Should be >0
